  ...
  <header class="bg-gray-900 sm:flex sm:items-center sm:justify-between xl:bg-white">
    ...
    >
      <div class="hidden xl:block xl:relative xl:max-w-xs xl:w-full">
        ...
        <input
          class="block w-full border border-transparent bg-gray-200 focus:outline-none focus:bg-white focus:border-gray-300 text-gray-900 rounded-lg ps-10 pe-4 py-2"
      ...
      <div class="sm:flex sm:items-center">
        <ul class="px-2 pt-2 pb-5 border-b border-gray-800 sm:flex sm:border-b-0 sm:py-0 sm:px-0">
          <nuxt-link v-for="navObj in navData" :key="navObj.link" :to="localePath(navObj.link)">
            <li
              class="block px-3 py-1 rounded font-semibold text-white hover:bg-gray-800 sm:text-sm sm:px-2 xl:text-gray-900 xl:hover:bg-gray-200"
            >
              {{ $t(navObj.label) }}
            </li>
          </nuxt-link>
